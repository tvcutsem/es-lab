...
\subsection{Stratified non-OO APIs}
...
\subsection{Comparison with other Stratified OO Intercession APIs}
...
\textbf{Smalltalk}
% TODO: refer to TS dispatching classes (Reflective Facilities in Smalltalk-80 (Foote and Johnson))
...
\begin{table*}
  ...
  \begin{tabular}{|p{0.15\textwidth}|p{0.15\textwidth}|p{0.15\textwidth}|p{0.15\textwidth}|p{0.15\textwidth}|p{0.15\textwidth}||p{0.15\textwidth}}
    ...
    \hline
    Funneling Trap
    & Y (get, through shifting) % Javascript
    & Y (invoke, trivially) % Java
    & Y (invoke, through shifting) % AmbientTalk
    & Y (handler is a non-methodical object) % E
    & Y (match clause, trivially) % E non-methodical objects
    & Y (only external operation on object is message sending) % Smalltalk
    \\
    \hline
    Uniform intercession
    % can all values in the language be virtualized?
    % link with PBH 'entity selection'?
    ...
    % opposite of selective interception
    % link with PBH 'operation selection'?

...
Our intercession API requires proxy handlers to implement 12 traps. A common use case for proxy handlers is to, for example, perform an access control check before performing the operation on some wrapped object. Only if the check succeeds is the operation is forwarded. A straightforward implementation would duplicate the pattern of access checking and forwarding in each trap. Ideally, if all operations would be funneled through a single trap, the handler would only have to perform the access check once, in the single trap.

We can achieve such funneling of all operations through a single trap by treating a proxy handler \emph{itself} as a proxy. In meta-programming terms, this corresponds to shifting meta-levels twice: the handler for a proxy that represents a handler effectively resides at the meta-meta-level. To avoid the confusion that invariably follows from this type of level-shifting, let us introduce some terminology. The \emph{base-level proxy} is the proxy object that with which other regular application objects will interact. That proxy's handler is the \emph{base-level handler}. If the base-level handler is itself a proxy, it is \emph{also} a \emph{meta-level proxy}. The handler of a meta-level proxy is a \emph{meta-level handler}.
...
Another way of looking at it is as follows. At the base-level, Javascript computation is reified in the form of 12 distinct kinds of operations that are being applied to objects as computation progresses. At the meta-level, all of these operations are reified as messages sent to handlers. Therefore, at the meta-meta-level, handlers need to reify only a single operation (property access), since all other operations have been recast in terms of property access.

Naturally, one can apply this trick of shifting meta-levels at the meta-meta-level itself. In fact, the API does not prevent one from shifting an arbitrary number of meta-levels. This legitimately brings up the question of infinite meta-regress. Such an infinite regress is avoided as long as a proxy handler is eventually implemented as a concrete Javascript object, rather than as yet another proxy.
...
% TODO: we should check which of our design principles holds true for these systems

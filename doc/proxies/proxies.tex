...
\begin{description}
  ...
  \item[Virtual objects] Proxies that \emph{emulate} a particular type of object, without that object ever having to be physically represented in the same address space. Examples include remote object proxies (where the ``wrapped'' object resides in another address space), persistent objects (where the represented object is stored in a database), transparent futures that act as placeholders for the value of asynchronous computations, etc.
\end{description}
...
The goal of the design principles discussed in this paper is to foster \emph{robust} metaprogramming APIs in the sense that they minimize interference with application-level code. We have applied these principles successfully to our API but have found them generally applicable to APIs for other languages or systems as well.

\section{Background: Reflection}
\label{sec:reflection}

Before we discuss the particulars of Javascript and our proposed Proxy API, we introduce some required terminology.
...
A limitation of the above caretaker abstraction is that objects exchanged via the caretaker are themselves not recursively wrapped in a revocable reference. In particular, if \texttt{Carol} defines a method that returns \texttt{this}, an unwrapped reference is exposed to \texttt{Bob}, circumventing the caretaker. This is an instance of the \emph{two-body problem}~\cite{eugster06uniform} -- the fact that the wrapping proxy and the wrapped object are distinct entities -- which may result in ``leaky'' wrappers. The abstraction discussed in the following section addresses this issue.

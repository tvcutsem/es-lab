...
We introduce a new meta-level API for Javascript based on dynamic proxies. Proxies have a wide array of use cases~\cite{eugster06uniform}. In this paper, we distinguish two general use cases depending on whether the proxy is proxying another object within the same physical address space:
...
\begin{description}
  ...
  \item[Virtual objects] Proxies that \emph{emulate} a particular type of object, without that object ever having to be physically represented in the same address space. Examples include remote object proxies (where the ``wrapped'' object resides in another address space), persistent objects (where the represented object is stored in a database), transparent futures that act as placeholders for the value of asynchronous computations, and so on.
\end{description}

The contributions of this paper are first, the introduction of a proxy-based metaprogramming API for Javascript (Section~\ref{sec:proxies_for_javascript}), second, to list a number of design principles that can be used to characterize general message-based object-oriented metaprogramming APIs (Sections~\ref{sub:wrappers} through \ref{sub:uniform_intercession}) and third, to provide an overview of related metaprogramming APIs in terms of these design principles (Section~\ref{sub:survey}).

Finally, we claim that the design principles put forward in this paper lead to \emph{robust} metaprogramming APIs in the sense that they minimize the impact on application-level code. We have applied these principles successfully to our API but believe they are generally applicable to APIs for other languages or systems.

\section{Background: Reflection}
\label{sec:reflection}

Before we discuss the particulars of Javascript and our proposed Proxy API, we introduce some terminology from the domain of reflection and metaprogramming.
...
Ferber~\cite{ferber89computational} distinguishes between \emph{structural} and \emph{behavioral} reflection, where the former reifies the structure of an object and the latter reifies the behavior of an object. Kiczales \emph{et. al}~\cite{kiczales91art} introduce a more fine-grained distinction. A MOP supports \emph{introspection} if it enables reflective access to the structure of an object. It supports \emph{self-modification} if it is possible to modify this structure. It further supports \emph{invocation} if it enables the reflective invocation of operations. Finally, it supports \emph{intercession} if it enables the creation of meta-objects with custom structure and behavior (enabling one to essentially change the semantics of certain operations of the programming language).
...
Without stratification, the intercession API pollutes the name space of the application.
...
InvocationHandler h = new InvocationHandler() {
  Object invoke(Object p,Method m,Object[] a){...}
};
...
In a PBH system, a single meta-object can control multiple base-level objects. The spatial scope of meta-objects is delimited using \emph{entity selection} and \emph{operation selection}: entity selection enables a meta-object to specify what objects it will control (e.g. all instances of a class, or only a particular instance of a class). Operation selection determines what particular operations of the affected objects will be reified (e.g. only field access). PBH even enables intra-operation selection, where reification will only occur if the operation satisfies further conditions (e.g. only reify calls to the \texttt{foo} method). A PBH system additionally enables \emph{temporal selection}, which controls the time during which a meta-object is active.

Reflex differs from our proxy approach in that it enables the creation of meta-objects that can act upon arbitrary existing objects. Nevertheless, some aspects of our proxy API can be understood in terms of PBH. Proxies induce a static form of entity selection: operations on proxy objects are reified, operations on non-proxy objects are not. Proxies may support temporal selection. For example, Javascript proxies only reify while they are in a trapping state. Once fixed, the proxy no longer reifies operations. Finally, proxies enable a static form of operation selection: some operations on proxies (e.g. \texttt{typeof}) are never reified, whereas others such as property access are always reified.

...
The goal of the design principles discussed in this paper is to foster \emph{robust} metaprogramming APIs in the sense that the API tries to minimize the interference with application code. We have used these principles to characterize our own API, but have found them generally applicable to meta-level APIs for other systems as well.
...
The Proxy API proposed in this paper is about achieving intercession, also known as \emph{behavioral reflection}~\cite{ferber89computational}. We will use the term \emph{intercession API} to refer to any API, framework or language feature that enables the creation of new base-level objects with custom meta-level behavior.

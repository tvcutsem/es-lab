...
Ferber~\cite{ferber89computational} makes a more coarse-grained distinction between \emph{structural} and \emph{behavioral} reflection. The Proxy API proposed in this paper enables the latter type of reflection. We will use the term \emph{intercession API} to refer to any API that enables the creation of new base-level objects with custom meta-level behavior.
...
Bracha and Ungar~\cite{bracha04mirrors} introduce the principle of stratification for mirror-based architectures. The principle highlights the importance of separating base and meta-level code, and is described in more detail in Section~\ref{sub:mirrors}. While Bracha and Ungar focus mostly on the benefits of stratification for \emph{structural} reflection, in this paper we focus on the benefits for \emph{behavioral} reflection.

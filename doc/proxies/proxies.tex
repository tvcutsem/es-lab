...
\textbf{Immutability:} if (parts of) an object can become immutable, the intercession API should be equipped with a mechanism to fix (parts of) a proxy. Fixed operations are no longer intercepted, and thus enable the language runtime to enforce the stronger invariants associated with immutability.

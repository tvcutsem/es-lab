...
We describe the design of a new metalevel API for Javascript that enables the creation of generic proxies for objects.
...
Before we discuss the particulars of Javascript and our proposed Proxy API, we introduce some terminology from the domain of metaprogramming.
...
A metaobject protocol~\cite{kiczales91art} is an object-oriented framework that describes the behavior of an object-oriented system. It is a term most commonly associated with reflective object-oriented programming languages.
...
The first three categories are typically well-supported by most meta-level APIs. Java's \texttt{java.lang.reflect} package, for example, supports introspection, invocation and limited self-modification. However, meta-level APIs that additionally support intercession are rare in mainstream languages. The CLOS metaobject protocol is the foremost example of a metaobject protocol supporting intercession~\cite{kiczales91art}.

It is customary for a metaobject protocol to represent language operations defined on objects as method invocaitons on their meta-objects. Throughout the rest of this paper, we use the term ``operation'' to characterize operations such as message sending, property access, assignment, method definition, performing an \texttt{instanceof} operation, and so on.

In a pure object-oriented language, the dominant operation applied to objects is the message send (or method invocation). It is therefore unsurprising to observe that, historically, many object-oriented languages have provided ``ad hoc'' intercession of method invocations. For example, in Smalltalk, the class \texttt{Object} defines a method named \texttt{doesNotUnderstand:}. When the Smalltalk VM cannot find a method selector in an object's class or superclasses, it invokes the object's \texttt{doesNotUnderstand:} method instead, passing along a reified message object~\cite{ducasse99evaluatingmessage}. While this approach is simple and flexible, we describe its drawbacks in section~\ref{sub:intercession_in_javascript}.
...
The use cases of proxies that can intercept base-level language operations can be distinguished primarily based on whether the proxy is proxying another object within the same physical address space:

\begin{description}
  \item[Generic wrappers] Proxies that wrap existing objects. Useful for creating access control wrappers, collecting profiling and code coverage information, implementing adaptors that intermediate between different versions of an API, and so on.
  \item[Virtual objects] Proxies that \emph{emulate} a particular type of object, without that object ever having to be physically represented in the same address space. Examples include remote object proxies (where the ``wrapped'' object resides in another address space), persistent objects (where the represented object is stored in a database), transparent futures~\cite{eugster06uniform} that act as placeholders for the value of asynchronous computations, and so on.
\end{description}
...
While the full Javascript language as we know it today has a lot of accidental complexity as a side-effect of a complex evolutionary process, at its core it is actually a simple dynamic language with first-class lexical closures and an concise object literal notation that makes it easy to create one-off, anonymous, objects. This simple core is what Crockford refers to as ``the good parts''~\cite{crockford08good}.
...
ECMAScript 5 defines a new object-manipulation API that provides programmers with more fine-grained control over the nature of object properties~\cite{ECMA262}. In Javascript, objects are records of \emph{properties} mapping names (strings) to values. A simple two-dimensional diagonal point can be defined as:
...
ECMAScript 5 distinguishes between two kinds of properties. Here, \texttt{x} is a \emph{data property}, mapping a name to a value directly. \texttt{y} is an \emph{accessor property}, mapping a name to a ``getter'' and/or a ``setter'' function. The expression \texttt{point.y} implicitly calls the getter.

ECMAScript 5 further associates with each property a set of \emph{attributes}. Attributes are meta-data that describe whether the property is writable (i.e.~can be assigned to), enumerable (~i.e. whether it appears in \texttt{for-in} loops) or configurable (~i.e. whether the property can be deleted and whether its attributes can be modified). The following code snippet shows how these attributes can be inspected and defined:
...
The \texttt{pd} object and the third argument to \texttt{defineProperty} are called \emph{property descriptors}. These are objects that describe properties of objects. Data property descriptors declare a \texttt{value} property, while accessor property descriptors declare a \texttt{get} and/or a \texttt{set} property.
...
The problem with \texttt{doesNotUnderstand:} and its analogues is that the trap is not \emph{stratified}: it is defined in the same name space as the rest of the application code. The only way in which a trap is distinguished from a regular method is by its name. This violation of stratification can lead to confusion:

\begin{itemize}
  \item Say an object \texttt{o} purposely defines the trap to intercept calls. Since the trap is part of the object's interface, clients of the object can accidentally invoke the trap as if it were an application-level method. Since traps are plain methods, nothing prevents them from being invoked as a plain method. This confuses meta-level code, since the call ``originated'' from the base-level.
  \item Say an object \texttt{o} accidentally defines the trap, treating that method name as part of the application. The VM may then incidentally invoke the application method as if it were a trap. This confuses the base-level code, since the call ``originated'' from the meta-level.
\end{itemize}

The intercession API is polluting the name space of the application. Why shouldn't a Smalltalk program about e.g. natural language translators be able to define a method named \texttt{doesNotUnderstand:}? The reason that the lack of stratification has not really posed a significant problem \emph{in practice} is that Spidermonkey and Smalltalk define only one such special method. But the approach does not scale. What if we were to introduce additional such traps to intercept not only method invocation, but also property access, assignment, lookup, enumeration, etc.? The number of ``reserved method names'' would quickly grow out of control.

So far in our discussion, we have limited the confusion caused by the lack of stratification to the fact that a trap is invoked by the wrong code. There are other ways in which the trap method may conflict with other base-level code. For example, a common Javascript idiom is to enumerate all properties of an object. Say we want to populate the content of a drop-down list in an HTML form with id \texttt{id} with all of the properties of an object \texttt{obj}:
...
If \texttt{obj} defines \noSuchMethod{} for the sake of intercepting method invocations, this method will show up in the enumeration of \texttt{obj}'s properties, which is probably not what the code expects.

Another drawback of defining the trap as part of the proxy is that it becomes impossible to recursively trap invocations of the trap \emph{itself}. Say \texttt{p} is a proxy. The invocation \texttt{p.\_\_noSuchMethod\_\_('foo',[1])} itself cannot be intercepted. It is not possible to transparently insert another proxy \texttt{p2} in between the caller and \texttt{p} such that \texttt{p2}'s \noSuchMethod{} trap is invoked as \texttt{p2.\_\_noSuchMethod\_\_('\_\_noSuchMethod\_\_',['foo',[1]])}. We will show in Section~\ref{sub:doublelifting} that this pattern of recursively proxying proxies themselves is useful when proxies can trap more than just method calls.

\section{Javascript Proxies}
\label{sec:proxies_for_javascript}

We now describe an intercession API for Javascript. This API has officially been proposed for inclusion in the next version of the ECMAScript standard. Its draft specification is available online~\cite{harmony_proxies}.
...
To implement generic wrappers around existing objects, a useful handler is one that simply forwards all operations applied to the proxy to another target object. The particular operations that a particular generic wrapper wants to intercept can then be defined as extensions of this generic forwarding handler.
...
var proxy = Proxy.create(makeForwaringHandler(target),
                         Object.getPrototypeOf(target));
\end{lstlisting}

We will make use of this handler in Sections~\ref{sub:doublelifting} and \ref{sec:examples}.
...
% TODO: explain picture and introduce legend
...
Meta-level funneling is demonstrated in Section~\ref{sec:examples}.
...
The second example is that proxies have to decide upfront whether they will proxy a plain object or a function object. By requiring proxies to determine at creation time whether or not they will proxy for an object or a function, the language runtime is able to uphold important invariants that it would have to forfeit if it would have to consult the \texttt{handler}. For example the expression \texttt{typeof o} should evaluate to \texttt{"object"} if \texttt{o} is ``just'' an object and to \texttt{"function"} if \texttt{o} is a function. Also, functions always inherit from \texttt{Function.prototype}. The distinction between object and function proxies allows our API to enforce these invariants.
...
It turns out that our intercession API is comprised of 7 fundamental traps and 5 derived traps. If 7 traps are sufficient to faithfully emulate an object, why define 12 of them? The reason is efficiency: implementing a derived trap directly may be more efficient than using its default implementation based on the corresponding fundamental trap. In our particular design, as a rule of thumb, we introduced derived traps only if their implementation could intercept the operation with less allocations than the corresponding implementation based on the fundamental trap.
...
If \texttt{obj} is a proxy, this invariant is no longer guaranteed! \texttt{has} could return true and \texttt{getPropertyDescriptor} could nevertheless return \texttt{undefined}. For proxies, both the language runtime and programmers can no longer count on such invariants being upheld, although a ``correct'' handler should attempt to uphold them.
...
\section{Example: Access Control Wrappers}
\label{sec:examples}
...
Given this abstraction, \texttt{Alice} can now wrap \texttt{Carol} in a revokable reference, pass the caretaker to \texttt{Bob}, and hold on to the revoker such that the reference can later be severed.
...
A limitation of the above caretaker abstraction is that objects exchanged via the caretaker are themselves not recursively wrapped in a revocable reference. In particular, if \texttt{Carol} defines a method that returns \texttt{this}, this will expose an unwrapped reference to \texttt{Bob}, circumventing the caretaker. This is an instance of the \emph{two-body problem}~\cite{eugster06uniform}, the fact that the wrapping proxy and the wrapped object are distinct entities, which may result in ``leaky'' wrappers. The abstraction discussed in the following section specifically addresses this issue.
...
In this section we summarize a variety of object-oriented intercession APIs. For each of these APIs, we discuss whether and how they adhere to the design principles put forth in this paper. We do not claim that our summary is exhaustive, but we believe the most representative intercession APIs are covered. Each of the discussed APIs has its own peculiarities. We briefly discuss the surveyed mechanisms and then summarize which of our design principles they uphold in Table~\ref{tab:related_work}.
...
\begin{tabular}{|p{0.20\textwidth}|p{0.80\textwidth}|}
    ...
    Java proxies      & Objects (instances of interface types)\\
...
% TODO: should we discuss Encapsulation and Ontological correspondence, the two other mirror principles?
...
One could characterize Reflex as a meta-intercession API: using Reflex, a programmer can define many different specific intercession APIs, each with their own settings for entity, operation and temporal selection.
...
\begin{itemize}
  \item Stratification: are proxy traps defined on a separate handler object?
  ...
  \item Temporary intercession: can proxies become fixed or disabled?
  \item Meta-level encapsulation: is the handler encapsulated by the proxy?
  \item Uniform intercession: can one proxy all runtime values in the system?
\end{itemize}

\section*{Acknowledgements}

We thank the members of the ECMA TC-39 committee and the \texttt{es-discuss@mozilla.org} community for their comments and feedback on our Proxy API. Thanks to Brendan Eich, Andreas Gal and Dave Herman for their detailed comments and Andreas Gal for his work on the Tracemonkey implementation.

...
function makeMembrane(initTarget) {
  ...
  function wrap(target) {
    if (Object.isPrimitive(target)) {
      ...
      return target; // so don't bother wrapping them
    } 
    var baseHandler = makeForwardingHandler(target);
    var metaHandler = Proxy.create({
      get: function(rcvr, name) {
    ...
    if (typeof target === "function") {
      return Proxy.createFunction(metaHandler,
                                  wrapFunction(target));
    } else {
...
Note how the handler of each membrane wrapper wraps all values crossing the membrane in both directions: arguments to methods cross the membrane in one direction, returned values or thrown exceptions in the other direction.

Recall that this example makes use of meta-level funneling. The \texttt{metaHandler} resides at the meta-meta-level. Therefore, it knows that it need only implement a \texttt{get} trap, and it knows that whatever it returns from the \texttt{get} trap will be treated as a meta-level trap (a function) that will immediately be invoked. To be able to wrap the arguments of these invocations, the \texttt{get} trap returns a variable-argument function. When this function is invoked, the \texttt{get} trap can collect the arguments\footnote{In Javascript, \texttt{arguments} is an object that contains all arguments passed to a function. It is used to write variable-argument functions, and it is customary to convert this object into a proper array first.}

% TODO: explain: the 'get' trap only wraps meta-level arguments
% the function proxy's call trap wraps base-level arguments
...
\subsection{Comparison with other Stratified Intercession APIs}
...
%TODO: distinguish between message-oriented vs function-oriented object systems
% and leave CLOS out of the comparison
...
% TODO: include discussion on non-methodical objects
% def obj match [verb, args] { ... }

% TODO: summarize the table with Y/N only and place markers in the table that refer to the text
\begin{table*}
  \begin{center}
  \begin{tabular}{|p{0.15\textwidth}|p{0.15\textwidth}|p{0.15\textwidth}|p{0.15\textwidth}|p{0.15\textwidth}|p{0.15\textwidth}||p{0.15\textwidth}}
    \hline
    Principle & Javascript proxies & Java proxies & AmbientTalk mirages & E eventual proxies & E non-methodical objects & Smalltalk \texttt{doesNotUnderstand:} \\
    \hline
    Virtualizes
    & objects % Javascript
    & objects % Java
    & objects % AmbientTalk
    & eventual references % E eventual proxies
    & objects % E non-methodical objects
    & objects % Smalltalk
    \\
    \hline
    Stratification
    & proxy vs handler % Javascript
    & Proxy vs InvocationHandler % Java
    & mirage vs mirror % AmbientTalk
    & proxy vs ProxyHandler % E eventual proxies
    & non-reified handling % E non-methodical objects
    & not stratified % Smalltalk
    \\
    \hline
    Meta-level encapsulation % can handler be accessed from proxy
    %TODO: introduce this principle
    & Y % Javascript
    & N (\texttt{Proxy.getInvocationHandler(proxy)}) % Java
    & Y (\texttt{reflect: mirage} consults mirror factory) % AmbientTalk
    & Y % E eventual proxies
    & Y % E non-methodical objects
    & N % Smalltalk
    \\
    \hline
    ...
    & N % Java
    & N % AmbientTalk
    & Y (resolutionSlot) % E eventual proxies
    & N % E non-methodical objects
    & Y (replacing an object using \texttt{become}) % Smalltalk
    \\
    \hline
    Meta-level shift
    ...
    & N (handler cannot be an eventual proxy) % E eventual proxies
    & N (non-methodicals have no handler) % E non-methodical objects
    & N (can't trap invocations of \texttt{doesNotUnderstand} itself) % Smalltalk
    \\
    \hline
    Funneling
    ...
    & Y (handler is a non-methodical object) % E
    & Y (trivially) % E non-methodical objects
    & Y (only external operation on object is message sending) % Smalltalk
    \\
    \hline
    Uniform intercession %TODO: introduce this principle
    % can all values in the language be virtualized?
    & N (primitives) % Javascript
    & N (classes, primitives) % Java
    & Y (everything is an object or reference) % AmbientTalk
    % TODO: \multirow to collapse E cases
    & Y (everything is an object or reference) % E eventual proxies and non-methodicals
    & Y (everything is an object) % Smalltalk
    \\
    \hline
    Full intercession % what operations on a value can be intercessed
    % for those values that can be interceded on, are all applicable operations interceded?
    & N (prototype, typeof and identity are fixed) % Javascript
    & N (identity, instanceof, getClass, wait and notify are fixed) % Java
    & Y % AmbientTalk
    & N (identity is fixed, reference state is enforced) % E eventual proxies
    & N (identity, trademarks) % E non-methodical objects
    & Y (if the defining class does not inherit from \texttt{Object}) % Smalltalk
    \\
    \hline
    Fundamental vs Derived traps
    & Y (cf. table~\ref{tab:operations})% Javascript
    & N (just 1 trap) % Java
    & N (all fundamental) % AmbientTalk
    & Y (\texttt{send} versus \texttt{sendOnly}) % E eventual proxies
    & N (just 1 trap) % E non-methodical objects
    & N (just 1 trap) % Smalltalk
    \\
    \hline
  \end{tabular}
  \end{center}

...
\usepackage{rotating}% => for \begin{sidewaystable}...\end{sidewaystable} (landscape tables)
\usepackage{tabularx}
...
\authorinfo{Tom Van Cutsem\titlenote{Tom Van Cutsem is a Postdoctoral Fellow of the Research Foundation, Flanders (FWO). This work was carried out while on a Visiting Faculty appointment at Google, sponsored by Google and a travel grant from the FWO.}}{Vrije Universiteit Brussel\\Pleinlaan 2\\ Brussels, Belgium}{tvcutsem@vub.ac.be}
\authorinfo{Mark S. Miller}{Google Research\\1600 Amphitheatre Parkway\\Mountain View, CA, USA}{erights@google.com}

\date{}

\begin{document}

\conferenceinfo{DLS 2010,} {October 18, 2010, Reno/Tahoe, Nevada, USA.}
\CopyrightYear{2010}
\copyrightdata{978-1-4503-0405-4/10/10}
...
%\toappear{DLS 2010, October 2010, Reno, Nevada.}
\maketitle
...
We introduce a new meta-level API for Javascript based on dynamic proxies. Proxies have a wide array of use cases~\cite{eugster06uniform}. Two general cases can be distinguished depending on whether the proxy is proxying another object within the same address space:
...
%the identification that common message interception mechanisms lead to interference with application code which are avoided using a stratified Proxy API (Sections~\ref{sub:intercession_in_javascript} and \ref{sub:stratification}).

%msm: Is fourth really a novel contribution? Has no one observed this before?
...
%It further supports \emph{explicit invocation} if it enables the reflective invocation of operations. 
...
\begin{table*}[t]
  \begin{center}
    \begin{small}
    \begin{tabular}{|l|p{0.40\textwidth}|p{0.38\textwidth}|}
    \hline
...
%msm: I think the above text is fine as is, but privately, a fine point is that the handler becomes unnecessary post fixing, and so the link may be dropped by the implementation. This distinction will actually become observable once we have weak references.
...
% TODO: advantages for internal spec. consistency, no overhead on non-proxy objects
% contrast with __get__, __set__, ...
...
% Changed the example above, since a frozen accessor can still return different values for a get.
...
The downside of this implementation is that \texttt{getProperty\-Descriptor} must needlessly allocate a property descriptor object if the named property exists. A custom implementation of \texttt{has} can often avoid this allocation.

%Note that it's possible to create an ``abstract'' prototype object that implements all derived traps in terms of the more fundamental traps. Since a handler is just a regular object, it can inherit the default derived traps from this abstract prototype, provided it implements the fundamental traps.
...
The membrane abstraction in Javascript enables innovative new ways of composing code from untrusted third parties on a single web page (so-called ``mash-ups''). Assuming the code is written in a safe subset of Javascript, such as Caja~\cite{miller08caja}, loading the untrusted code using a membrane-wrapped \texttt{eval} function makes it possible to isolate scripts from one another and from their container page.
...
%Progress on this extension can be tracked at \url{https://bugzilla.mozilla.org/show_bug.cgi?id=546590}}.
...
\centering
%\begin{tabular}{|p{0.07\textwidth}|p{0.09\textwidth}|p{0.09\textwidth}|p{0.09\textwidth}|p{0.09\textwidth}|p{0.09\textwidth}|p{0.09\textwidth}|p{0.09\textwidth}|p{0.09\textwidth}|p{0.09\textwidth}|}
\begin{tabularx}{\textwidth}{|p{0.07\textwidth}|X|X|X|X|X|X|X|X|X|}
    \hline
    \textbf{API} &
    \raggedright \textbf{Values that can be virtualized} &
    \textbf{Stratification} &
    \raggedright \textbf{Temporary Intercession} &
    \textbf{Meta-level shifting} &
    \textbf{Meta-level funnelling} &
    \raggedright \textbf{Selective interception} &
    \raggedright \textbf{Fundamental vs. Derived Traps} &
    \raggedright \textbf{Handler Encapsulation} &
    \textbf{Uniform intercession}\\
    \hline
    \raggedright \textbf{Javascript proxies} &
    \raggedright Objects and functions, not primitives &
    \raggedright Yes, proxy versus handler &
    \raggedright Yes, by fixing the proxy &
    \raggedright Yes, if handler is itself a proxy &
    \raggedright Yes, via the \texttt{get} trap, for a meta-handler &
    \raggedright Yes, identity, prototype and \texttt{typeof} are fixed &
    \raggedright Yes, see Table~\ref{tab:operations} &
    \raggedright Yes &
    No, primitives can't be virtualized\\
    \hline
    \raggedright \textbf{Java proxies} &
    \raggedright Objects whose static type is an interface or \texttt{Object}, not primitives &
    \raggedright Yes, \texttt{Proxy} versus \texttt{Invocation\-Handler}&
    \raggedright No, proxies remain proxies forever&
    \raggedright Yes, if handler is itself a proxy&
    \raggedright Yes, via the \texttt{invoke} trap, for any handler&
    \raggedright Yes, identity, class, interfaces and final methods inherited from \texttt{Object}&
    \raggedright No, \texttt{invoke} is the only trap&
    \raggedright No, because of \texttt{Proxy.get\-Invocation\-Handler(proxy)}&
    No, instances of non-interface classes and primitives can't be virtualized\\
    \hline
    \raggedright \textbf{Eugster's Uniform Java proxies} &
    \raggedright Objects, not primitives &
    \raggedright Yes, \texttt{Proxy} versus \texttt{Invocation\-Handler} and \texttt{AccessHandler}&
    \raggedright No, proxies remain proxies forever&
    \raggedright Yes, if invocation- and accesshandler are themselves a proxy&
    \raggedright Yes, via the \texttt{invoke} trap, for a meta-invocation- and accesshandler &
    \raggedright Yes, identity, class and interfaces are fixed (final fields/methods can be virtualized)&
    \raggedright No, all traps are fundamental &
    \raggedright No, because of \texttt{Proxy.get\-Invocation\-Handler(proxy)}&
    No, primitives can't be virtualized\\
    \hline
    \raggedright \textbf{Ambient\-Talk mirages} &
    \raggedright Objects and eventual references &
    \raggedright Yes, mirage versus mirror &
    \raggedright No, mirages remain mirages forever &
    \raggedright Yes, if mirror is itself a mirage &
    \raggedright Yes, via the \texttt{invoke} trap, for a meta-handler &
    \raggedright Yes, an object's mirror is determined by a mirror factory, not by the mirage &
    \raggedright No, all traps are fundamental &
    \raggedright Yes, but can access the mirror indirectly via a mirror factory &
    Yes, all values are objects or references (mirages virtualize both)\\
    \hline
    \raggedright \textbf{E proxies} &
    \raggedright Eventual references &
    \raggedright Yes, proxy versus \texttt{ProxyHandler} &
    \raggedright Yes, by \emph{resolving} the proxied reference into a real reference &
    \raggedright No, handler cannot itself be an eventual reference proxy &
    \raggedright Yes, if handler is a non-methodical object, via its \texttt{match} clause &
    \raggedright Yes, identity and the ``state'' of a reference are fixed &
    \raggedright Yes, \texttt{sendOnly} can be derived from \texttt{send} &
    Yes &
    Yes, all values are objects or references (proxies virtualize references)\\
    \hline
    \raggedright \textbf{E non-methodicals} &
    Objects &
    \raggedright Yes, non-methodicals have no behavior other than their \texttt{match} clause &
    \raggedright No, non-methodicals remain non-methodicals forever &
    \raggedright No, non-methodicals have no handler &
    \raggedright Yes, via the \texttt{match} clause &
    \raggedright Yes, identity and ``trademarks'' (unforgeable types) are fixed &
    \raggedright No, the \texttt{match} clause is the only trap &
    Yes &
    Yes, all values are objects or references (non-methodicals virtualize objects)\\
    \hline
    \raggedright \textbf{Smalltalk} \texttt{doesNot\-Understand:} &
    Objects &
    \raggedright No, proxy and handler are the same object &
    \raggedright Yes, by replacing the object using Smalltalk's \texttt{become:} primitive &
    \raggedright No, cannot trap invocations of \texttt{doesNot\-Understand:} itself &
    \raggedright Yes, via \texttt{doesNot\-Understand:} itself, if class implements or inherits no other methods &
    \raggedright Yes, calls to methods implemented in or inherited by the defining class will not be intercepted &
    \raggedright No, the \texttt{doesNot\-Understand:} method is the only trap &
    \raggedright No, proxy and handler are the same object &
    Yes, all values are objects\\
    \hline
    \raggedright \textbf{ParcPlace Smalltalk dispatching classes} &
    Objects &
    \raggedright Yes, dispatching classes define only \texttt{dispatch\-Message:} &
    \raggedright Yes, by replacing the object using Smalltalk's \texttt{become:} primitive &
    \raggedright Yes, by sending a \texttt{dispatch\-Message:} explicitly to a dispatching class &
    \raggedright Yes, via the \texttt{dispatch\-Message:} method &
    \raggedright No, since all interaction is via messages, and all messages are trapped &
    \raggedright No, the \texttt{dispatch\-Message:} method is the only trap &
    Yes &
    Yes, all values are objects\\
    \hline
  \end{tabularx}
...
\begin{description*}
  ...
  \item[Meta-level funnelling:] the ability to funnel all operations through a single trap.
...
\bibitem[Bracha and Griswold(1993)]{bracha93strongtalk}
G.~Bracha and D.~Griswold.
\newblock Strongtalk: typechecking smalltalk in a production environment.
\newblock In \emph{OOPSLA '93: Proceedings of the eighth annual conference on
  Object-oriented programming systems, languages, and applications}, pages
  215--230, NY, USA, 1993. ACM.
\newblock ISBN 0-89791-587-9.
\newblock \doi{http://doi.acm.org/10.1145/165854.165893}.

\bibitem[Bracha and Ungar(2004)]{bracha04mirrors}
G.~Bracha and D.~Ungar.
\newblock Mirrors: Design principles for meta-level facilities of
  object-oriented programming languages.
\newblock In \emph{OOPSLA '04: Proceedings of the 19th annual Conference on
  Object-Oriented Programming, Systems, Languages and Applications}, pages
  331--343, 2004.

\bibitem[Bracha et~al.(2010)Bracha, Ah\'e, Bykov, Kashai, Maddox, and
  Miranda]{bracha10newspeak}
G.~Bracha, P.~Ah\'e, V.~Bykov, Y.~Kashai, W.~Maddox, and E.~Miranda.
\newblock Modules as objects in newspeak.
\newblock In \emph{ECOOP '10: Proceedings of the 24th European Conference on
  Object Oriented Programming, Maribor, Slovenia}, LNCS. Springer Verlag, June
  2010.

\bibitem[Brant et~al.(1998)Brant, Foote, Johnson, and Roberts]{brant98wrappers}
J.~Brant, B.~Foote, R.~E. Johnson, and D.~Roberts.
\newblock Wrappers to the rescue.
\newblock In \emph{ECOOP '98: Proceedings of the 12th European Conference on
  Object-Oriented Programming}, pages 396--417, London, UK, 1998.
  Springer-Verlag.
\newblock ISBN 3-540-64737-6.

\bibitem[Crockford(2008)]{crockford08good}
D.~Crockford.
\newblock \emph{Javascript: The Good Parts}.
\newblock O'Reilly, 2008.

\bibitem[Ducasse(1999)]{ducasse99evaluatingmessage}
S.~Ducasse.
\newblock Evaluating message passing control techniques in smalltalk.
\newblock \emph{Journal of Object-Oriented Programming (JOOP)}, 12:\penalty0
  39--44, 1999.

\bibitem[{ECMA International}(2009)]{ECMA262}
{ECMA International}.
\newblock \emph{ECMA-262: ECMAScript Language Specification}.
\newblock ECMA, Geneva, Switzerland, fifth edition, December 2009.
\newblock URL
  \url{http://www.ecma-international.org/publications/standards/Ecma-327.htm}.

\bibitem[Eugster(2006)]{eugster06uniform}
P.~Eugster.
\newblock Uniform proxies for java.
\newblock In \emph{OOPSLA '06: Proceedings of the 21st annual conference on
  Object-oriented programming systems, languages, and applications}, pages
  139--152, NY, USA, 2006. ACM.
\newblock ISBN 1-59593-348-4.
\newblock \doi{http://doi.acm.org/10.1145/1167473.1167485}.

\bibitem[Foote and Johnson(1989)]{foote89reflective}
B.~Foote and R.~E. Johnson.
\newblock Reflective facilities in smalltalk-80.
\newblock In \emph{OOPSLA '89: Conference proceedings on Object-oriented
  programming systems, languages and applications}, pages 327--335, NY, USA,
  1989. ACM.
\newblock ISBN 0-89791-333-7.
\newblock \doi{http://doi.acm.org/10.1145/74877.74911}.

\bibitem[Gal(2006)]{gal06efficient}
A.~Gal.
\newblock \emph{Efficient bytecode verification and compilation in a virtual
  machine}.
\newblock PhD thesis, University of California, Irvine, Long Beach, CA, USA,
  2006.

\bibitem[Kiczales et~al.(1991)Kiczales, Rivieres, and Bobrow]{kiczales91art}
G.~Kiczales, J.~D. Rivieres, and D.~G. Bobrow.
\newblock \emph{The Art of the Metaobject Protocol}.
\newblock MIT Press, Cambridge, MA, USA, 1991.
\newblock ISBN 0262111586.

\bibitem[Miller et~al.(2005)Miller, Tribble, and Shapiro]{miller05concurrency}
M.~Miller, E.~D. Tribble, and J.~Shapiro.
\newblock Concurrency among strangers: Programming in {E} as plan coordination.
\newblock In \emph{Symposium on Trustworthy Global Computing}, pages 195--229.
  Springer, 2005.

\bibitem[Miller(2006)]{miller06robust}
M.~S. Miller.
\newblock \emph{Robust Composition: Towards a Unified Approach to Access
  Control and Concurrency Control}.
\newblock PhD thesis, John Hopkins University, Baltimore, Maryland, USA, May
  2006.
\balancecolumns
\bibitem[Miller(2010)]{miller10et}
M.~S. Miller.
\newblock Weak maps proposal for {ECMAScript Harmony}, 2010.
\newblock \url{tinyurl.com/weak-maps}.

\bibitem[Miller and Reid(2009)]{eproxies}
M.~S. Miller and K.~Reid.
\newblock Proxies for eventual references in {E}, 2009.
\newblock \url{wiki.erights.org/wiki/Proxy}.

\bibitem[Miller et~al.(2008)Miller, Samuel, Laurie, Awad, and
  Stay]{miller08caja}
M.~S. Miller, M.~Samuel, B.~Laurie, I.~Awad, and M.~Stay.
\newblock Caja: Safe active content in sanitized javascript, June 2008.
\newblock \url{tinyurl.com/caja-spec}.

\bibitem[{Mostinckx} et~al.(2007){Mostinckx}, {Van Cutsem}, {Timbermont}, and
  {Tanter}]{mostinckx07behavioral}
S.~{Mostinckx}, T.~{Van Cutsem}, S.~{Timbermont}, and E.~{Tanter}.
\newblock Mirages: Behavioral intercession in a mirror-based architecture.
\newblock In \emph{Proceedings of the Dynamic Languages Symposium - OOPSLA'07
  Companion}, pages 222--248. ACM Press, 2007.

\bibitem[Redell(1974)]{redell74naming}
D.~D. Redell.
\newblock \emph{Naming and Protection in Extensible Operating Systems}.
\newblock PhD thesis, Department of Computer Science, University of California
  at Berkeley, Nov 1974.

\bibitem[Tanter et~al.(2003)Tanter, Noy{\'e}, Caromel, and Cointe]{tanter03pbh}
{\'E}.~Tanter, J.~Noy{\'e}, D.~Caromel, and P.~Cointe.
\newblock Partial behavioral reflection: spatial and temporal selection of
  reification.
\newblock In \emph{OOPSLA '03: Proceedings of the 2003 Conference on
  Object-Oriented Programming Systems, Languages and Applications}, pages
  27--46. ACM, 2003.

\bibitem[Ungar and Smith(1987)]{ungar87power}
D.~Ungar and R.~B. Smith.
\newblock Self: The power of simplicity.
\newblock In \emph{OOPSLA '87: Conference proceedings on Object-oriented
  Programming Systems, Languages and Applications}, pages 227--242. ACM Press,
  1987.
\newblock ISBN 0-89791-247-0.
\newblock \doi{http://doi.acm.org/10.1145/38765.38828}.

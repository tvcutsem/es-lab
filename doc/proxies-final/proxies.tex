...
\begin{tabularx}{\textwidth}{|p{0.07\textwidth}|X|X|X|X|X|X|X|X|X|}
    ...
    \raggedright Yes, \texttt{Proxy} versus \texttt{InvocationHandler}&
    ...
    \raggedright Yes, \texttt{Proxy} versus \texttt{InvocationHandler} and \texttt{AccessHandler}&
    \raggedright No, proxies remain proxies forever&
    ...
    \hline
    \raggedright \textbf{Smalltalk} \texttt{doesNotUnderstand:} &
    Objects &
    ...
    \raggedright No, cannot trap invocations of \texttt{doesNotUnderstand:} itself &
    \raggedright Yes, via \texttt{doesNotUnderstand:} itself, if class implements or inherits no other methods &
    \raggedright Yes, calls to methods implemented in or inherited by the defining class will not be intercepted &
    \raggedright No, the \texttt{doesNotUnderstand:} method is the only trap &
    \raggedright No, proxy and handler are the same object &
    Yes, all values are objects\\
    \hline
    \raggedright \textbf{ParcPlace Smalltalk dispatching classes} &
    Objects &
    \raggedright Yes, dispatching classes define only \texttt{dispatchMessage:} &
    \raggedright Yes, by replacing the object using Smalltalk's \texttt{become:} primitive &
    \raggedright Yes, by sending a \texttt{dispatchMessage:} explicitly to a dispatching class &
    \raggedright Yes, via the \texttt{dispatchMessage:} method &
    \raggedright No, since all interaction is via messages, and all messages are trapped &
    \raggedright No, the \texttt{dispatchMessage:} method is the only trap &
    Yes &
    Yes, all values are objects\\
    \hline
  \end{tabularx}

...
A purse's {\tt depositWhen} method is simply a convenient wrapper around the purse's {\tt deposit} method. Following the sequence of events starting at the {\tt depositWhen} method on line 13,
\begin{enumerate}
\item[14] {\tt depositWhen} uses {\tt then} to register a callback for calling this purse's own {\tt deposit} method \emph{when} the argument promise has been fulfilled. The {\tt depositWhen} method returns a promise for what the {\tt deposit} method will return.
...
\item[10] {\tt deposit} looks up the alleged payment purse in the {\tt amp} table. If this is anything other than a purse of the same currency at the same bank, this lookup will instead return undefined, causing the following function call to throw an error.

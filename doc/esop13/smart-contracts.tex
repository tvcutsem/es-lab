...
\bnote{Tom}{Should we give an overview of the paper here?}
...
\bnote{Tom}{Preceding sentence feels lost. Transition too abrupt?}
...
\bnote{Tom}{Least-authority: explain or remove?}
...
JavaScript is a complex language, but this paper depends only on a small subset with two core constructs, \emph{functions} and \emph{records}. For the sake of brevity, this paper borrows one syntactic convenience proposed for ES6, \emph{arrow functions}~(``{\tt =>}''), and one proposed for ES7, the \emph{eventual-send operator}~(``{\tt !}'').\bnote{Tom}{ES6,ES7: ES5 abbreviation no longer introduced. Also, dependency on strict-mode?}
 Expanding away these conveniences, all the code here is working ES5 code, and is available at \myurl{code.google.com/p/es-lab/source/browse/trunk/src/ses/\#ses} and its {\tt contract} subdirectory.
...
\item[{\tt var amp = WeakMap()}] assigns to {\tt amp} a new empty weak map. WeakMaps are an ES6 extension (emulated by SES on ES5 browsers). Ignoring space usage, {\tt amp} is simply an object-identity-keyed table. {\tt amp.set(obj, val)} associates {\tt obj}'s identity as key with {\tt val} as value, so {\tt amp.get(obj)} returns {\tt val}, {\tt amp.has(obj)} returns true, and {\tt amp.delete(obj)} removes this entry. These methods use only {\tt obj}'s identity without interacting with {\tt obj}.
...
Historically, two broad strategies for avoiding this tragedy have emerged: a governance strategy and a property rights strategy~\cite{smith_exclusion_2002}. Each has its counterpart in computational systems: access control lists implement a governance strategy, while object-capability systems pursue a property rights strategy.
...
Object references as rights have certain properties that distinguish them from the more familiar rights in society. These differences highlight certain dimensions in which rights can differ. Object references are sharable, specific, opaque, and exercisable. We contrast this with money~\cite{fox_property_2008} as a right that is exclusive, fungible, measurable, and symbolic.

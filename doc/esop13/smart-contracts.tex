...
\paragraph{Arrow functions.} The following five lines all define a one parameter function which returns twice its parameter. All bind a local variable named ``{\tt double}'' to this function. The first two lines are of historic interest only. This paper uses only the arrow function syntax of the last three lines.
...
\item Alice's money is fully vulnerable to misbehavior by the bank. In the following escrow exchange scenario, Alice is more protected, but at the price of vulnerability to \emph{three} third-party services. We return to this issue below.
\end{itemize}

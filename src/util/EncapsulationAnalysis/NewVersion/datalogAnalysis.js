 ...
 */
 this.genConstraintsScopeNumber = function genConstraintsScopeNumber(ast, annotationsObj,  encodeVar, encodeField, encodeLoc, encodeCon, getNextTempVar, getNextAstNumber, astNumberMap, relName, makeConstraint,  functionClones, nClones, maxArity) {
	...
	// console.log(s);
	 ...
	 if(lhsType === 'IdExpr' && rhsType === 'LiteralExpr'){
		 ...
		 retConstraints.push(makeConstraint('loadDot',[tempVarPrim,rhsExpr[2][1].name,'toString']));
		 retConstraints.push(makeConstraint('callerCon',[outerNum,tempVarPrim,1]));
		 retConstraints.push(makeConstraint('actualArgCon',[tempVarPrim,0,rhsExpr[2][1].name,1]));
		 retConstraints.push(makeConstraint('actualRetCon',[tempVarPrim,'$$dump',1]));
		 
		 tempVarPrim = getNextTempVar();
		 retConstraints.push(makeConstraint('loadDot',[tempVarPrim,rhsExpr[2][1].name,'valueOf']));
		 retConstraints.push(makeConstraint('callerCon',[outerNum,tempVarPrim,1]));
		 retConstraints.push(makeConstraint('actualArgCon',[tempVarPrim,0,rhsExpr[2][1].name,1]));
		 retConstraints.push(makeConstraint('actualRetCon',[tempVarPrim,'$$dump',1]));	 
	     }
	     if (rhsExpr[3][0] === 'IdExpr'){
		 tempVarPrim = getNextTempVar();
		 retConstraints.push(makeConstraint('loadDot',[tempVarPrim,rhsExpr[3][1].name,'toString']));
		 retConstraints.push(makeConstraint('callerCon',[outerNum,tempVarPrim,1]));
		 retConstraints.push(makeConstraint('actualArgCon',[tempVarPrim,0,rhsExpr[3][1].name,1]));
		 retConstraints.push(makeConstraint('actualRetCon',[tempVarPrim,'$$dump',1]));
	     ...
	     if (rhsExpr[2][0] === 'IdExpr'){
		 retConstraints.push(makeConstraint('assign',[lhsExpr[1].name,rhsExpr[2][1].name]));
	     }
	     if (rhsExpr[3][0] === 'IdExpr'){
		 retConstraints.push(makeConstraint('assign',[lhsExpr[1].name,rhsExpr[3][1].name]));
	     }
	     return retConstraints;
	 }
	     ...
	     return retConstraints;
	      ...
	      if (rhsExpr[2][0] === 'MemberExpr'){
		 tempVarPrim = getNextTempVar();
		 retConstraints.push(makeConstraint('loadDot',[tempVarPrim,rhsExpr[2][3][1].name,'toString']));
		 retConstraints.push(makeConstraint('callerCon',[outerNum,tempVarPrim,1]));
		 retConstraints.push(makeConstraint('actualArgCon',[tempVarPrim,0,rhsExpr[2][3][1].name,1]));
		 retConstraints.push(makeConstraint('actualRetCon',[tempVarPrim,'$$dump',1]));
		 
		 tempVarPrim = getNextTempVar();
		 retConstraints.push(makeConstraint('loadDot',[tempVarPrim,rhsExpr[2][3][1].name,'valueOf']));
		 retConstraints.push(makeConstraint('callerCon',[outerNum,tempVarPrim,1]));
		 retConstraints.push(makeConstraint('actualArgCon',[tempVarPrim,0,rhsExpr[2][3][1].name,1]));
		 retConstraints.push(makeConstraint('actualRetCon',[tempVarPrim,'$$dump',1]));	 
	     }
	     else{
		 throw 'genConstraintsExpr - Delete should only be applied to member epressions -> ' + rhsExpr[2][0];
	     }
	     return retConstraints;
	 }
		 ...
		 num = rhsExpr.num;
		 constraint = makeConstraint('newPrimObj',[tempVar,num]);
		 retConstraints.push(constraint);
		 if(rhsExpr[3][0] === 'LiteralExpr'){
		     ...
		     tempVarPrim = getNextTempVar();
		     retConstraints.push(makeConstraint('loadDot',[tempVarPrim,rhsExpr[3][1].name,'toString']));
		     retConstraints.push(makeConstraint('callerCon',[outerNum,tempVarPrim,num]));
		     retConstraints.push(makeConstraint('actualArgCon',[tempVarPrim,0,rhsExpr[3][1].name,num]));
		     retConstraints.push(makeConstraint('actualRetCon',[tempVarPrim,'$$dump',num]));
		 ...
		 // Case: 3[..] = x 
		// According to the spec any property assignment to a transient object is an error in strict mode
		/* tempVar = getNextTempVar();
		 num = lhsExpr.num;
		 ...
		 if(lhsExpr[3][0] === 'LiteralExpr'){
		   ...
		   //  constraint = makeConstraint('storeDot', [tempVar, annotate(lhsExpr[3][1].value, retConstraints), rhsExpr[1].name]);
		    // retConstraints.push(constraint);
		     ...
		     retConstraints.push(makeConstraint('callerCon',[outerNum,tempVarPrim,num]));
		     retConstraints.push(makeConstraint('actualArgCon',[tempVarPrim,0,lhsExpr[3][1].name,num]));
		     retConstraints.push(makeConstraint('actualRetCon',[tempVarPrim,'$$dump',num]));
		 ...
		 if(lhsExpr[3][0] === 'LiteralExpr'){
		   ...
		   //  constraint = makeConstraint('storeDot', [lhsExpr[2][1].name, annotate(lhsExpr[3][1].value, retConstraints), rhsExpr[1].name]);
		    // retConstraints.push(constraint);
		     ...
		     if(lhsExpr[3][1].value === "length"){
			 tempVarPrim = getNextTempVar();
			 retConstraints.push(makeConstraint('loadDot',[tempVarPrim,rhsExpr[1].name,'toString']));
			 retConstraints.push(makeConstraint('callerCon',[outerNum,tempVarPrim,num]));
			 retConstraints.push(makeConstraint('actualArgCon',[tempVarPrim,0,rhsExpr[1].name,num]));
			 retConstraints.push(makeConstraint('actualRetCon',[tempVarPrim,'$$dump',num]));
			 
			 
			 tempVarPrim = getNextTempVar();
			 retConstraints.push(makeConstraint('loadDot',[tempVarPrim,rhsExpr[1].name,'valueOf']));
			 retConstraints.push(makeConstraint('callerCon',[outerNum,tempVarPrim,num]));
			 retConstraints.push(makeConstraint('actualArgCon',[tempVarPrim,0,rhsExpr[1].name,num]));
			 retConstraints.push(makeConstraint('actualRetCon',[tempVarPrim,'$$dump',num]));
		     ...
		     if(lhsExpr[1].hasOwnProperty('annotation')){
	  ...
	  //   console.log("invoke num is " + num);
		...
		// retConstraints.push(constraint);
		     constraint = makeConstraint('callerCon',[outerNum,rhsExpr[2][1].name,num]);
		     retConstraints.push(constraint);
		     for(i = 4 ; i < len ; i++){
			 if (rhsExpr[i][0] === 'IdExpr'){
			     constraint  = makeConstraint('actualArgCon',[rhsExpr[2][1].name,i-4,rhsExpr[i][1].name,num]);
			     retConstraints.push(constraint);
			    // constraint = makeConstraint('storeDot',[tempVar,'$A$Num',rhsExpr[i][1].name]);
			    // retConstraints.push(constraint);
			 }
		   ...
		   //  retConstraints.push(constraint);
		     ...
		     constraint = makeConstraint('actualRetCon',[rhsExpr[2][1].name,lhsExpr[1].name,num]);
		     retConstraints.push(constraint);
		...
		// retConstraints.push(constraint);
		     ...
		     constraint = makeConstraint('instance',[lhsExpr[1].name,num,rhsExpr[2][1].name,num]);
		     retConstraints.push(constraint);
		     
		     constraint = makeConstraint('callerCon',[outerNum,rhsExpr[2][1].name,num]);
		     retConstraints.push(constraint);
		     
		     constraint  = makeConstraint('actualArgCon',[rhsExpr[2][1].name,0,lhsExpr[1].name,num]);
		     retConstraints.push(constraint);
		     
		     for(i = 3 ; i < len ; i++){
			 if (rhsExpr[i][0] === 'IdExpr'){
			     constraint  = makeConstraint('actualArgCon',[rhsExpr[2][1].name,i-2,rhsExpr[i][1].name,num]);
			     retConstraints.push(constraint);
	     ...
	     tempVar = getNextTempVar();
	     prototypeNum = getNextAstNumber();
	     retConstraints.push(makeConstraint('newObj',[tempVar,prototypeNum]));
	     retConstraints.push(makeConstraint('hPtsTo',[num,'prototype',prototypeNum]));
	 ...
	 case 'Program': case 'BlockStmt': case 'CatchClause':
	     retConstraints.push(makeConstraint('assign',['this_0_1','window_0_1']));
	     for (i = 2;i <len; i++){

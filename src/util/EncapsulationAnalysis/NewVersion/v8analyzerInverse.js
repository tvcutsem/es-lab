...
var MdRules = require('./datalogInverseRules.js');
var MdField = require('./datalogFieldNames.js');
var MdNative = require('./nativeInverseDatalog.js');
//var MdPrecious = require('./preciousDatalog.js');
var MdAttacker = require('./attackerInverseDatalog.js');
var MdDOM = require('./domDatalog.js');

//var MdSu = require('./suDatalog.js');
var MdAnnotations = require('./annotationsAdsafe.js');
var fs = require('fs')
...
var  annotationsObj = MdAnnotations.annotationsObj;
...
Object.prototype.toString = 
	...
	var renameCloneAst = MrenameClone.renameCloneControlStrip(renderControlStripAst(text),getNextAstNumber, astNumberMap,functionClones, nClones, maxArity)
 ...
 //    var preciousConstraints = MdPrecious.genPreciousConstraints(encodeVar, encodeField, encodeLoc,encodeCon);
       ...
       var annotationConstraints = MdAnnotations.genAnnotationConstraints(renameCloneAst,annotationsObj,encodeVar, encodeField, encodeLoc,encodeCon, getNextTempVar, getNextAstNumber, astNumberMap, relName, makeConstraint, functionClones, nClones, maxArity);	
...
//       result = result + preciousConstraints + '\n';
   ...
   var aPointsTo = [];
   function renderAttackerPointsTo(text,showNativeFlag){
	var stmts = text.split('\n');
	var i = 1;
	var words = [];
	var wordsLen =0;
		...
		if((words[j][0] === '0') || (words[j][0] === '1') || (words[j][0] === '2') || (words[j][0] === '3') || (words[j][0] === '4') || (words[j][0] === '5') || (words[j][0] === '6') || (words[j][0] === '7') || (words[j][0] === '8') || (words[j][0] === '9') ){
		   // console.log(words[j]);
			...
			if(showNativeFlag === 1){
			    ...
			    if((astNumberMap[decodeLoc(words[j])].isAttacker != 1) && (astNumberMap[decodeLoc(words[j])].isNative != 1)){
			    	aPointsTo.push(astNumberMap[decodeLoc(words[j])]);
			    }  
			}
		    }
   ...
   function basicAnalyzer(inputFileName, outputFileName, cloneNativeFlag, cloneDOMFlag, cloneUserFlag, callSiteConstraints, state){
	 ...
	 //  datalogCode = renderDatalog(source,freezeProtoFlag,cloneNativeFlag,cloneDOMFlag,cloneUserFlag,callSiteConstraints,1); // attackerFlag = 1, 
       ...
       child = exec('java -Xmx2048m -cp ./BddbddbAnalyzer/: net.sf.bddbddb.Solver ' + './BddbddbAnalyzer/output.datalog', 
		    { encoding: 'utf8'
		      , timeout: 0
			...
			if (error !== null) {
			    ...
			    if((state === 1) && (cloneUserFlag != 0)){
			      var callClonedOutSource =  fs.readFileSync('./BddbddbAnalyzer/Results/callClonedOut.tuples','ascii');
			      var callSiteConstraints = genCallSiteConstraints(callClonedOutSource);
			      console.log('Basic Analyzer: Phase 1 completed');
			      return basicAnalyzer(inputFileName, outputFileName, cloneNativeFlag, cloneDOMFlag,0, callSiteConstraints,2);
			  }
			    else{
				var aPointsToSource =  fs.readFileSync('./BddbddbAnalyzer/Results/AActualPtsTo.tuples','ascii');
				var provenanceSource = fs.readFileSync('./BddbddbAnalyzer/Results/Provenance.tuples','ascii');
				var provenanceResult = renderAttackerPointsTo(provenanceSource,0);
				var aPointsToResult = renderAttackerPointsTo(aPointsToSource,0);
				result = result + 'Functions and DOM objects that the attacker can potentially hold are: \n\n';
				result = result + aPointsToResult;
				result = result + '\n\n';
				result = result + 'FUNCTIONS CALLED IN ORDER TO REACH PRECIOUS: \n\n';
				result = result + provenanceResult;
				
				console.log(result);
				fs.writeFileSync(outputFileName,result,encoding='ascii');   
...
child =   exec('rm ./BddbddbAnalyzer/Results/*',
  function (error, stdout, stderr) {
      if (error !== null) {
	  //  sys.print('stdout: ' + stdout);
	  sys.print('stderr: ' + stderr);
	  ...
	  basicAnalyzer(inputFileName, outputFileName, cloneNativeFlag, cloneDOMFlag, 0, "", 1);
      }
  });
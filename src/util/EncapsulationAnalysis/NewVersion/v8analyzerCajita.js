...
var  annotationsObj = MdAnnotations.annotationsObj;
 ...
 function makeEncoderDecoderVar(){
	 ...
	 if (strToNum.hasOwnProperty(temp)){
	    ...
	    // console.log(s);
	     if(String(s).search('_0_') >= 0){
	//	 console.log(s);
		 var t = s.search('_0_');
		 toGlobalField.push([s,String(s).slice(0,t)]);
		 toGlobalVar.push([String(s).slice(0,t),s]);
	     }
	    // console.log(s);
             strToNum[temp] = count;
             numToStr[count] = s;
...
*/
   ...
   function basicAnalyzer(inputFileName, outputFileName, cloneNativeFlag, cloneDOMFlag, cloneUserFlag, callSiteConstraints, state){
	 ...
	 //  datalogCode = renderDatalog(source,freezeProtoFlag,cloneNativeFlag,cloneDOMFlag,cloneUserFlag,callSiteConstraints,1); // attackerFlag = 1, 
       ...
       child = exec('java -Xmx2048m -cp ./BddbddbAnalyzer/: net.sf.bddbddb.Solver ' + './BddbddbAnalyzer/output.datalog >./BddbddbAnalyzer/outputBuffer', 
		    { encoding: 'utf8'
		      , timeout: 0
		      , maxBuffer: 2000*1024
		      , killSignal: 'SIGKILL'

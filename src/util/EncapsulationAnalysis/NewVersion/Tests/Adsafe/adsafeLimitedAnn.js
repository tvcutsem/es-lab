...
// The selector is * or /
                ...
                selector = {
                    op: match[1]  // BOX-ANNOTATE: CHECK whether this is an $A$All
                };
            } else if (match[2]) {
...
// Step through each selector.
        ...
        for (i = 0; i < query.length; i += 1) {
            ...
            name = selector.name;
            func = hunter[$A$All(selector.op)]; // BOX-ANNOTATE Check this annotation (can use hasOwnProperty on hunter)
...
// the :first and :rest selectors because they are so simple.
                ...
                flipflop = false;
                func = pecker[$A$All(selector.op)]; // BOX-ANNOTATE Check this annotation (can use hasOwnProperty on pecker)
                if (typeof func !== 'function') {
                    switch (selector.op) {
                    case ':first':
...
// Return an ADsafe dom object.
        ...
        dom = {
            q: function (text) {
                ...
                if (typeof hunter[$A$All(query[0].op)] !== 'function') { //BOX-ANNOTATE Check this annotation

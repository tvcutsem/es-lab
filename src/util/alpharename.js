 ...
 * @param opt_readCaps an array of AST nodes that should be preserved as-is in
 *   read contexts.
 * @param opt_writeCaps an array of AST nodes that should be preserved as-is.
 * @return a copy of rootAst with identifiers renamed.  Any nodes in
 *   opt_readCaps or opt_writeCaps will not be renamed if they appear in the
 *   appropriate context.
 ...
 */
function alphaRename(rootAst, namer, opt_readCaps, opt_writeCaps) {
...
function isReadContext(parentAst, ast) {
  if (!parentAst) { return false; }  // conservative.
  switch (parentAst[0]) {
    case 'CountExpr': return false;
    case 'AssignExpr': return ast !== parentAst[2];
    default: return true;
  }
}
...
function renameOne(parentAst, parentAlphaMap, ast, letScopedNames) {
  if (opt_writeCaps && opt_writeCaps.indexOf(ast) >= 0) { return ast; }
  if (opt_readCaps && opt_readCaps.indexOf(ast) >= 0) {
    if (!isReadContext(parentAst, ast)) {
      throw new Error('${' + renderEcmascript(ast) + '} used in assignment.');
    }
    return ast;
  }

...
(function loggerModule() {
  ...
  ses.startTimer = function(name) {
    if (ses.stopTimer !== null) {
      throw new Error('Already timing');
    }
    ses.counters = Object.create(null);

    ses.stopTimer = function() {
      var stopTime = Date.now();
      console.profileEnd();
      ses.logger.log(name + ' took ' + (stopTime - startTime) + 'ms');

      function compare(name1, name2) {
        return ses.counters[name2] - ses.counters[name1];
      }
      var names = gopn(ses.counters).sort(compare);
      names.forEach(function(name) {
        ses.logger.log(name + ': ' + ses.counters[name]);
      });
  ...
  ses.count = function(name) {
    if (name in ses.counters) {
      ses.counters[name]++;
    } else {
      ses.counters[name] = 1;
    }
